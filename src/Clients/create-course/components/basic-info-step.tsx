...
import { createCourse, updateCourse } from "../services/createCourse.service";
import { useAuth } from "@/auth/context/AuthContext";
import { toast } from "react-toastify";
import type { ICategory } from "@/types/Courses";
import { useParams } from "react-router-dom";
import { viewCourseById } from "@/Clients/myCourse/services/myCourses.service";
...
export function BasicInfoStep({ onCreated }: BasicInfoStepProps) {
  ...
  const parseId = id ? Number(id) : null
  const isEditing = !!parseId

  // ONE SET OF STATES for both create and edit
  ...
  useEffect(() => {
    if (!parseId || !accessToken) return  // only run if editing

    const fetchCourse = async () => {
      setLoading(true)
      try {
        const data = await viewCourseById({ courseId: parseId, accessToken })
        console.log("data for course:" , data)
        const course = data.courses 

        setTitle(course.title_en || "")
        setDescription(course.description || "")
        setWhatYouLearn(course.what_you_learn || "")
        setSelectedCategory(course.category[0]?.id.toString() || "")
  ...
  // HANDLE SUBMIT - handles both create and update
  const handleSubmit = async () => {
    if (isEditing) {
      ...
      if (!createdCourseId) {
        toast.error("Course created but id was not returned")
        return
      }
    ...
    <section className="mx-auto max-w-180">
      <header className="mb-8 space-y-2">
        <h1 className="text-4xl font-bold tracking-tight text-[#1b2430]">
          {isEditing ? 'Edit Course' : 'Course Info'}
        </h1>
